...
export default function BlogList() {
  const { blogs, fetchBlogs, deleteBlog, isLoading } = useBlogs();
  ...
  const columns = [
      ...
      render: (row) => (
        <div className="flex gap-1 flex-wrap">
          {row.tags?.slice(0, 2).map((tag, i) => (
            <span key={i} className="text-xs bg-white/5 px-2 py-1 rounded-md text-gray-300">{tag}</span>
          ))}
          {row.tags?.length > 2 && (
            <span className="text-xs text-gray-500">+{row.tags.length - 2} more</span>
          )}
        </div>
      )
    },
    {
      header: 'Author',
      accessor: 'author',
      render: (row) => <span className="text-sm text-gray-300">{row.author}</span>
    },
    {
      header: 'Status',
      accessor: 'isActive',
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400">Manage your company articles and news.</p>
        </div>
        <Link 
          to="/admin/blogs/new" 
      ...
      <DataTable columns={columns} data={blogs} keyField="_id" />
