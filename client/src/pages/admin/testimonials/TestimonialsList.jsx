...
export default function TestimonialsList() {
  const { testimonials, fetchTestimonials, deleteTestimonial, isLoading } = useTestimonials();
  ...
  const columns = [
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">{row.authorRole} {row.authorCompany ? `@ ${row.authorCompany}` : ''}</p>
          </div>
        </div>
      )
    },
    {
      ...
      accessor: 'quote',
      render: (row) => (
        <p className="text-gray-400 text-sm max-w-xs truncate" title={row.quote}>
          "{row.quote}"
        </p>
      )
    },
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <Link 
            to={`/admin/testimonials/${row._id}/edit`}
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable columns={columns} data={testimonials} keyField="_id" />
